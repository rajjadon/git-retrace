...
suite('Launchpad', () => {
  ...
  /** The whole integration suite runs against one fixed workspace folder (the shared fixture repo) — this adds a remote to it for the duration of `fn`, then removes it, so other suites relying on "no remote" are unaffected. */
  async function withOriginRemote<T>(url: string, fn: () => Promise<T>): Promise<T> {
    execFileSync('git', ['remote', 'add', 'origin', url], { cwd: manifest.repoRoot });
    try {
      return await fn();
    } finally {
      execFileSync('git', ['remote', 'remove', 'origin'], { cwd: manifest.repoRoot });
    }
  }
