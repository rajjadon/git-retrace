...
test('contributes.commands: titles are namespaced so the command palette groups them', () => {
  for (const cmd of commands) {
    assert.match(cmd.title, /^GitSense: /, `${cmd.command} has an un-namespaced title: ${cmd.title}`);
  }
});
...
test('commandPalette: copySha is hidden, because there is no commit selected there', () => {
  ...
  assert.ok(hidden.includes(COMMANDS.copySha), 'gitsense.copySha must be hidden from the command palette');
});
